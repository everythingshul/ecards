// ---------------------------------------------------------------------------
// disccardpromos.com gift card provider — the ONE integration this platform
// uses for every org (unlike email, gift cards are NOT per-org configurable;
// there is a single disccardpromos account/API for the whole system).
//
// This module is the only place in the app that talks to disccardpromos.com.
// Every call site in routes/cards.js goes through the functions below, so if
// the real API contract differs once confirmed (or they add an endpoint we
...
// and is what routes/cards.js expects regardless of mode. Their docs host
// (docs.disccardpromos.com) blocked automated fetching from this build
// environment, so the exact endpoint paths/payloads below are a best-guess
// placeholder pending confirmation with their team.
// ---------------------------------------------------------------------------
